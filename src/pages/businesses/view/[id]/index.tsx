...
import { deleteRoomById } from 'apiSdk/rooms';
...
function BusinessViewPage() {
  ...
  const roomHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('room', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/rooms/create?business_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.room?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/rooms/view/${record.id}`)} key={record.id}>
                            <Td>{record.name}</Td>
                            <Td>{record.status}</Td>
                            <Td>
